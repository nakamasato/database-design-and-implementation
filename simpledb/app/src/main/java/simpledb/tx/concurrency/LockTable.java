...
public class LockTable {
  ...
  public synchronized void sLock(BlockId blk) {
    System.out.println("[LockTable] starting slock on " + blk.number());
    try {
    ...
    System.out.println("[LockTable] completed slock on " + blk.number());
  }
  ...
  synchronized void xLock(BlockId blk) {
    System.out.println("[LockTable] starting xlock on " + blk.number());
    try {
      long timestamp = System.currentTimeMillis();
    ...
    System.out.println("[LockTable] completed xlock on " + blk.number());
  }

  synchronized void unlock(BlockId blk) {
    System.out.println("[LockTable] starting unlock on " + blk.number());
    int val = getLockVal(blk);
    if (val > 1)
      locks.put(blk, val - 1);
    else {
    ...
    System.out.println("[LockTable] completed unlock on " + blk.number());
  }
  ...
  private int getLockVal(BlockId blk) {
    ...
    System.out.println("[LockTable] getLockVal for " + blk.number() + " is " + ival);
